...
pub fn vault_address(create_key_path: String) -> eyre::Result<()> {
    // Load create key
    let create_key = solana_sdk::signature::read_keypair_file(&create_key_path).map_err(|e| {
        eyre::eyre!(
            "Failed to read create key '{}': {}",
            create_key_path,
            e
        )
    })?;
...
pub fn show_multisig(create_key_path: String, url: &str) -> eyre::Result<()> {
    ...
    let account = rpc
        ...
        .map_err(|e| eyre::eyre!("Failed to fetch multisig account at {}: {}", multisig_pda, e))?;
    ...
    println!();
    println!("Threshold:      {}/{}", multisig_data.threshold, multisig_data.members.len());
